...
export const optimizeImageForMobile = (imageUrl, options = {}) => {
  ...
  if (!imageUrl || typeof imageUrl !== 'string' || !imageUrl.includes('imagedelivery.net')) {
    return imageUrl;
  }
...
export const getMobileSizes = (variant = 'default') => {
  const sizeMap = {
    hero: '(max-width: 768px) 428px, 100vw',
    gallery: '(max-width: 768px) 428px, 50vw',
    thumbnail: '(max-width: 768px) 200px, 25vw',
    card: '(max-width: 768px) 350px, 33vw',
    default: '(max-width: 768px) 428px, 50vw'
  };
...
export const shouldOptimizeForMobile = (imageUrl) => {
  ...
  if (typeof window === 'undefined') return false;
  return window.innerWidth <= 768 && imageUrl && imageUrl.includes('imagedelivery.net');
};