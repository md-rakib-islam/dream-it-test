...
} from "../../../utils/mobileImageOptimizer";
import {
...
const OptimizedImage = ({
  ...
  const optimizeImageUrl = (url) => {
    ...
    try {
      ...
      if (typeof window !== "undefined" && window.innerWidth <= 768) {
        ...
        const optimalWidth = isMobile ? 380 : 768;
  ...
  try {
    ...
    const presetConfig =
      IMAGE_OPTIMIZATION_PRESETS[variant] ||
      IMAGE_OPTIMIZATION_PRESETS.gallery_large;
    ...
    const optimizedSrc = generateOptimizedImageUrl(src, {
      ...
      height: height || 600,
      quality: priority
        ? Math.max(presetConfig.quality, 90)
    ...
    if (shouldOptimizeForMobile(finalSrc)) {
      const mobileSrc = optimizeImageForMobile(finalSrc, {
        ...
        height: height || 240,
        quality: priority
          ? Math.max(presetConfig.quality, 85)
          : presetConfig.quality,
        priority,
      });
  ...
  const imageProps = {
    ...
    onError: handleError,
    quality: priority
      ? Math.min(Math.max(quality, 90), 95)
      : Math.min(quality, 85), // Higher quality for better UX
    priority,
